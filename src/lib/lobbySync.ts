...
// Indice di appartenenza per-utente: users/{uid}/lobbyMemberships/{lobbyId} → {lobbyId}
...
export async function joinLobbyByCode(uid: string, displayName: string, code: string): Promise<Lobby> {
  const safeName = displayName.trim().slice(0, 40)
  const snap = await getDocs(query(lobbiesCol(), where('code', '==', code.toUpperCase().trim()), where('isActive', '==', true)))
...
export async function sendMessage(
  ...
  const [lobbySnap, mSnap] = await Promise.all([getDoc(lobbyDoc(lobbyId)), getDoc(memberDoc(lobbyId, uid))])
...
export async function transferGMRole(uid: string, lobbyId: string, targetUid: string): Promise<void> {
  ...
  const [lobbySnap, targetMemberSnap] = await Promise.all([getDoc(lobbyDoc(lobbyId)), getDoc(memberDoc(lobbyId, targetUid))])
  if (!lobbySnap.exists()) throw new Error('Lobby non trovata.')
  const lobby = docToLobby(lobbySnap)
  const effectiveGmUid = lobby.gmUid ?? lobby.ownerId
...
export async function getMessagesAfter(uid: string, lobbyId: string, after?: number, pageLimit = 50): Promise<LobbyMessage[]> {
    ...
    lobbyIds.map(async (lobbyId) => {
      try {
        ...
        if (member.lastSeenAt) {
          const unreadSnap = await getDocs(query(messagesCol(lobbyId), where('sentAt', '>', Timestamp.fromMillis(member.lastSeenAt))))
...
export function subscribeToMessages(lobbyId: string, onMessages: (msgs: LobbyMessage[]) => void): Unsubscribe {
...
export function subscribeToMembers(lobbyId: string, onMembers: (members: LobbyMember[]) => void): Unsubscribe {
